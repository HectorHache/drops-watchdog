...
**Status:** v2.5 — **Phases 0-2 COMPLETE (gates met)** — live digest delivered to t.me/dropwatchdog; next: Phase 3 (site)
...
| **3 — Site** | `drops.json` + `index.html` (sections, priority toggle, filters, countdowns, watch-minutes, history, polling, _headers, RSS) + CF Pages + DNS (B9 wizard) | ego-local-html-verify E2E pass; filters/toggle/sections/countdown/poll proven |
...
| **5 — Favorites (Steam)** | `steam_import.py` (Mick API + household2 browser scrape, surprise preserved), in-site selector → favorites; ⭐ sorting; Steam library badge; **24/7 personal DM alert for favorites** | Favorite game drop triggers instant DM (even off-hours); selector round-trips; badge shows |
