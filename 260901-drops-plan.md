...
| **5 — Favorites (Steam)** | `steam_import.py` (Mick API + wifey browser scrape, surprise preserved), in-site selector → favorites; ⭐ sorting; Steam library badge; **24/7 personal DM alert for favorites** | Favorite game drop triggers instant DM (even off-hours); selector round-trips; badge shows |
