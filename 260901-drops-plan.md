...
**Status:** v3.3 — **Phases 0-6 DONE + round-trip VERIFIED + Phase 7 (learning loop) DONE (2026-09-01)** — favicon pack wired (media/->docs/ deterministic + branded PWA manifest, 23 tests), recency-decayed interaction signals (favorite=10/like=3/remind=5, 30-day half-life) -> favoriteWeights in drops.json, site priority sort by learned weight + 🔥 hot badge (score>=8), weekly recap 'Most interacted' section. Delta Force already learned 10 pts from live KV favorite (weight 10->19). Optional remaining: GH Actions mirror (needs Twitch token as Actions secret — Mick decision), more PWA polish, README. (KV binding live: POST /api/interact → ok:true; GET /api/interactions returns both; kv_pull consumed 2 → watermark 1788292504928; Delta Force favorite already present (manual) → INSERT OR IGNORE dedupe proven; kv_interaction events recorded; 18 tests green). Remaining: only OPTIONAL Phase 7 (learning loop on historical data, GH Actions mirror, PWA polish, README). (Steam both libraries imported; household2=162 via auth scrape, mick=1339 API; favorites CLI + site tab; Steam badge live; 24/7 personal DM cron; CLOSED-persistence fix). Next: Phase 6 (in-site click-to-favorite via CF Functions + KV, weekly recap, GH Actions mirror, PWA).: Steam import live (mick API = 1339 games, badge on 11/102 campaigns), favorites CLI + site Favorites tab, 24/7 personal DM cron (`twitch-drops-personal`, verified Delta Force <24h DM sent), CLOSED-persistence fix (no flip-flop). Remaining: household2 friend-shared scrape (needs Mick's Steam login + Guard code), in-site selector round-trip (Phase 6 KV).
...
| D4 | Favorites | **Steam-driven**: Mick (`steamcommunity.com/id/d4rkwar/`, Steam Web API) + household2 (`profiles/REDACTED-THIRD-PARTY-ID`, friend-shared, authenticated browser scrape — surprise preserved), then an in-site **selector** |
...
│   ├── steam_import.py    ← Phase 5: API (Mick) + browser scrape (household2) → steam_games.json
...
| **5 — Favorites (Steam)** ✅ **DONE (2026-09-01)** | `steam_import.py` (Mick API **DONE** = 1339 games; household2 browser scrape **PENDING** — needs Mick's Steam login + Guard); favorites CLI + site Favorites tab **DONE**; Steam library badge **DONE** (11/102 live); **24/7 personal DM alert DONE** (cron `twitch-drops-personal` `0,30 * * * *`, Delta Force alert verified in DM); in-site selector round-trip = Phase 6 KV | Favorite drop triggers instant DM even off-hours ✓; selector round-trips → Phase 6; badge shows ✓ |
| **6 — Interaction + learning** ✅ **DONE + VERIFIED (2026-09-01)** — KV round-trip proven end-to-end; **Phase 7 learning loop DONE**: `src/learning.py` (signals + weights, decayed), site priority sort + 🔥 badge, weekly recap extras, favicon pack + branded manifest | `functions/api/interact.js` (POST ⭐/👍/remind → KV) + `functions/api/interactions.js` (GET → watchdog pulls) at REPO ROOT (`/functions`, NOT docs/ — static-root trap); `src/kv.py` pulls each sync → favorites apply within 30 min; SPA buttons + weekly stats panel + ES/EN toggle + PWA-lite (manifest+sw); crons `drops-weekly-recap` (Sun 19:00) + `drops-health` (Mon 09:00); GH Actions CI (tests on push). KV binding: create namespace `DROPS_KV` + bind to Pages project → then `env.DROPS_KV` live | KV interaction → next sync picks it up (code ready); wizard step pending |
...
**Phase 5:**
6. **Steam login in ego-browser** (you have 2FA — have the Steam Guard code ready when I prompt). Needed only for household2's scrape; your own profile works via API key alone.
