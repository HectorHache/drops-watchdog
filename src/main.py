...
def cmd_steam_import(args):
    """Import Steam libraries (mick API + household2 browser scrape) into steam_games."""
...
def cmd_steam_pick(args):
    """List, interactive select, or auto-seed favorites from Mick's Steam library."""
    conn = dbm.init_db(db_path())
    if args.auto is not None or getattr(args, "non_interactive", False):
        ...
        import steam_picker as sp
        picker = sp.SteamFavoritesPicker(conn, page_size=getattr(args, "page_size", 20), auto_push=not args.dry)
        picker.run()
...
def main():
    ...
    si_cmd = sub.add_parser("steam-import", help="import Steam libraries (mick API + household2 scrape)")
    si_cmd.add_argument("--owners", default="mick,household2", help="comma list: mick,household2")
    si_cmd.add_argument("--dry", action="store_true", help="no site push")
    sp = sub.add_parser("steam-pick", help="interactive selector or auto-seed favorites from Steam")
    sp.add_argument("--auto", type=float, default=None, help="auto-favorite all games with >= N hours playtime")
    sp.add_argument("--limit", type=int, default=40, help="max games to display in non-interactive list")
    sp.add_argument("--page-size", type=int, default=20, help="games per page in interactive mode (default: 20)")
    sp.add_argument("--non-interactive", action="store_true", help="run non-interactive list output")
